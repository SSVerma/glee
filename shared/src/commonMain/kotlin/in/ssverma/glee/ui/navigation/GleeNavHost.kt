package `in`.ssverma.glee.ui.navigation

import androidx.compose.runtime.*
import androidx.compose.ui.Modifier
import androidx.navigation3.runtime.NavEntry
import androidx.navigation3.runtime.NavKey
import androidx.navigation3.runtime.rememberNavBackStack
import androidx.navigation3.ui.NavDisplay
import androidx.savedstate.serialization.SavedStateConfiguration
import `in`.ssverma.glee.domain.model.ModelDownloadStatus
import `in`.ssverma.glee.ui.chat.ChatScreen
import `in`.ssverma.glee.ui.chat.ChatViewModel
import `in`.ssverma.glee.ui.models.ModelManagementScreen
import `in`.ssverma.glee.ui.splash.SplashScreen
import `in`.ssverma.glee.ui.settings.SettingsScreen
import kotlinx.serialization.modules.SerializersModule
import kotlinx.serialization.modules.polymorphic
import kotlinx.serialization.modules.subclass

private val GleeNavConfig = SavedStateConfiguration {
    serializersModule = SerializersModule {
        polymorphic(NavKey::class) {
            subclass(Splash::class)
            subclass(ModelManagement::class)
            subclass(Chat::class)
            subclass(ManageSkills::class)
            subclass(Settings::class)
        }
    }
}

@Composable
fun GleeNavHost(
    viewModel: ChatViewModel,
    modifier: Modifier = Modifier
) {
    val uiState by viewModel.uiState.collectAsState()
    
    val backStack = rememberNavBackStack(configuration = GleeNavConfig, Splash)

    NavDisplay(
        backStack = backStack,
        modifier = modifier
    ) { key ->
        val entry: NavEntry<NavKey> = when (key) {
            Splash -> NavEntry(key) {
                SplashScreen(onSplashComplete = {
                    backStack.clear()
                    if (uiState.availableModels.any { it.downloadStatus == ModelDownloadStatus.Downloaded }) {
                        backStack.add(Chat)
                    } else {
                        backStack.add(ModelManagement)
                    }
                })
            }
            ModelManagement -> NavEntry(key) {
                ModelManagementScreen(onClose = { if (backStack.size > 1) backStack.removeAt(backStack.size - 1) else { backStack.clear(); backStack.add(Chat) } })
            }
            Chat -> NavEntry(key) {
                ChatScreen(
                    viewModel = viewModel,
                    onModelManagement = { backStack.add(ModelManagement) },
                    onManageSkills = { backStack.add(ManageSkills) },
                    onSettings = { backStack.add(Settings) }
                )
            }
            ManageSkills -> NavEntry(key) {
                `in`.ssverma.glee.ui.skills.ManageSkillsScreen(onBack = { backStack.removeAt(backStack.size - 1) })
            }
            Settings -> NavEntry(key) {
                SettingsScreen(
                    onBack = { backStack.removeAt(backStack.size - 1) },
                    onModelManagement = { backStack.add(ModelManagement) },
                    onManageSkills = { backStack.add(ManageSkills) }
                )
            }
            else -> NavEntry(key) { }
        }
        entry
    }
}
