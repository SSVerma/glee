...
class OpenMapSkill(
    ...
    override suspend fun execute(input: String): Flow<SkillResult> = flow {
        emit(SkillResult.Progress("Parsing location query..."))
        try {
            ...
            emit(SkillResult.Progress("Opening map for: $query..."))
            ...
            if (success) {
                emit(SkillResult.Success("Successfully opened the map application for $query."))
            } else {
                emit(SkillResult.Error("Failed to open the map application."))
            }
        } catch (e: Exception) {
            emit(SkillResult.Error("Error processing map query: ${e.message}"))
        }
    }
}
